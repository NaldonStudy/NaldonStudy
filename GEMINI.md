...
* **커밋 전 사용자 승인 필수**: 어떠한 경우에도 사용자의 명시적인 승인 없이 `git commit`을 독단적으로 수행하지 마십시오. 커밋 전에는 항상 스테이징된 파일 목록과 제안할 커밋 메시지를 채팅을 통해 보고하고, 사용자가 "응", "커밋해" 등 승인 의사를 표시했을 때만 실행하십시오.
* **검증 항목**:
  1. 타입 체크 (`tsc --noEmit`)
  2. 린트 체크 (`eslint`)
  3. 빌드 체크 (`next build`)
